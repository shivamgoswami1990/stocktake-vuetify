    ...
    <v-card-text>
      <v-stepper v-model="stepValue" vertical>
        ...
        <v-stepper-step :complete="stepValue > 1" step="1">
          <p v-if="stepValue > 1 && selectedCustomer !== undefined && selectedCustomer !== null"
             class="font-weight-bold mb-0 text-subtitle-1">
            {{selectedCustomer.name + ', ' + selectedCustomer.city + ', ' + selectedCustomer.state_name}}
          </p>
          <div v-else>
            <span class="d-block"> Select a customer</span>
        ...
        <v-stepper-content step="1">
          <v-flex>
            <v-autocomplete hide-no-data v-model="selectedCustomer"
              ...
              <template slot="item" slot-scope="{ item, tile }">
                ...
                <v-list-item-content @click="selectedCustomer = item">
          ...
          <v-btn color="primary" @click="stepValue = 2"
        ...
        <v-stepper-step :complete="stepValue > 2" step="2">
          <p v-if="stepValue > 2 && selectedCompany !== undefined && selectedCompany !== null"
             class="font-weight-bold mb-0 text-subtitle-1">
            {{selectedCompany.name + ', ' + selectedCompany.city + ', ' + selectedCompany.state_name}}
          </p>
          <div v-else>
            <span class="d-block"> Select a company</span>
        ...
        <v-stepper-content step="2">
          <v-flex>
            <btn-group v-model="selectedCompany" :items="companies" mandatory/>
          </v-flex>

          <div class="ml-2 mt-4">
            <v-btn color="primary" @click="getLastCreatedInvoiceForCompany"
                   :disabled="checkSelectedCompany" :loading="companyButtonLoading">
              Continue
            </v-btn>
        ...
        <v-stepper-step :complete="stepValue > 3" step="3">
          <p v-if="stepValue > 3 && selectedPackaging !== undefined && selectedPackaging !== null"
             class="font-weight-bold mb-0 text-subtitle-1">
            Packaging - {{selectedPackaging.name}}
          </p>
          <div v-else>
            <span class="d-block"> Select a packaging</span>
        ...
        <v-stepper-content step="3">
          <v-flex>
            <btn-group v-model="selectedPackaging" :items="packaging_options"
                       :item-height="90" :item-width="140" is-item-icon mandatory/>
          </v-flex>
